...
typedef void (*ptr_AddHiresTexturePath)(const char* path);
typedef void (*ptr_RemoveHiresTexturePath)(const char* path);
#if defined(M64P_PLUGIN_PROTOTYPES)
...
EXPORT void CALL AddHiresTexturePath(char* path);
EXPORT void CALL RemoveHiresTexturePath(char* path);
#endif
